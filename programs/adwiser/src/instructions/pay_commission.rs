...
impl<'info> PayCommission<'info> {
    pub fn pay_commission_fn(ctx: Context<PayCommission>, percentage: u64) -> Result<()> {
        ...
        amount += (ctx.accounts.campaign_acc.no_of_txns + 1) * 5000;
        ...
        msg!("Before txn Adwiser Amount: {:?}",ctx.accounts.adwiser.lamports());
        ...
        msg!("AdWiser account: {:?}", ctx.accounts.adwiser.key());
        let cpi_ctx = CpiContext::new_with_signer(cpi_program, cpi_accounts, signer_seeds);
        system_program::transfer(cpi_ctx, amount)?;
        msg!("After txn Adwiser Balance: {:?}",ctx.accounts.adwiser.lamports());
        ...
        msg!("Pay Commission Txn successful");
        Ok(())
    }
}